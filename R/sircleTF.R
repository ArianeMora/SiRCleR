...
#'
#' @param filename Input file (= Output of rcm function).
...
sircleORA_TF <- function(filename, regLabels="RegulatoryLabels", emptyRegLabel="", RemoveBackgroundGenes="TRUE", enricher_geneID, enricher_Pathways, enricher_PathwayName="", fileType="pdf", minGSSize=10, maxGSSize=1000 , Plot_p.adj=0.2, Plot_Percentage=10, OutputFileName=''){
  ## ------------ Setup and installs ----------- ##
  ...
  for(g in grps_labels) {
    ...
    if (!(dim(clusterGoSummary)[1] == 0)){
      ...
      #Safe file
      write.csv(clusterGoSummary, paste("SiRCleRCM/SiRCleRCM_TF_", Sys.Date(), "/", OutputFileName,"ClusterGoSummary_", enricher_PathwayName, '-', g, ".csv", sep=""), row.names = FALSE)#Export the ORA results as .csv
      ...
      if (!(dim(clusterGoSummary_Select)[1] == 0)) {#exclude df's that have no observations
        ...
        ggsave(file=paste("SiRCleRCM/SiRCleRCM_TF_", Sys.Date(), "/", OutputFileName,"Dotplot_",g,"_" ,enricher_PathwayName, ".", fileType, sep=""), plot=Dotplot, width=10, height=8)
        plot(Dotplot)
        #2. Emapplot
        ...
        ggsave(file=paste("SiRCleRCM/SiRCleRCM_TF_", Sys.Date(), "/", OutputFileName,"Emapplot_", g,"_", enricher_PathwayName, ".", fileType, sep=""), plot=Emapplot, width=10, height=8)
         plot(Emapplot)
        #4. Upsetplot:
        ...
        ggsave(file=paste("SiRCleRCM/SiRCleRCM_TF_", Sys.Date(), "/", OutputFileName,"UpsetPlot_", g,"_", enricher_PathwayName, ".", fileType, sep=""), plot=UpsetPlot, width=10, height=8)
        plot(UpsetPlot)
      }
    }
  }
